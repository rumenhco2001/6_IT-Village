...
<div class="log_panel">
	...
	<form action="" method="post">
		...
		if (isset($_POST['log_in'])) {
			...
			if (!empty($username) && !empty($password)) {
				...
				if ($correct_data == 1) {
					...
					$read_query = "SELECT in_progress, played_games FROM users WHERE username = '{$_SESSION['username']}'";
					...
					if ($correct_username == 1) {
						...
						echo '<div class="error">'.'Invalid DATA!'.'</div>';
					}
				}
			} else {
		...
		?>
		<input class="btn log_btn" type="submit" name="log_in" value="Log in">
